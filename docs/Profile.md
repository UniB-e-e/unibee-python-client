...
[**new_apikey_post**](Profile.md#new_apikey_post) | **POST** /merchant/new_apikey | Merchant New ApiKey, The old key will expire in one hour
...
# **new_apikey_post**
> MerchantNewApikeyPost200Response new_apikey_post(body)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Merchant New ApiKey, The old key will expire in one hour
        api_response = api_instance.new_apikey_post(body)
        print("The response of Profile->new_apikey_post:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling Profile->new_apikey_post: %s\n" % e)
```
