...
class MerchantMetricListGet200ResponseData(BaseModel):
    """
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in merchant_metrics (list)
        _items = []
        if self.merchant_metrics:
            for _item in self.merchant_metrics:
                if _item:
                    _items.append(_item.to_dict())
            _dict['merchantMetrics'] = _items
        return _dict
