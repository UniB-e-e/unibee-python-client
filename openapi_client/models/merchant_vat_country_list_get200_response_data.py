...
class MerchantVatCountryListGet200ResponseData(BaseModel):
    """
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in vat_country_list (list)
        _items = []
        if self.vat_country_list:
            for _item in self.vat_country_list:
                if _item:
                    _items.append(_item.to_dict())
            _dict['vatCountryList'] = _items
        return _dict
